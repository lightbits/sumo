...
void main()
{
    ...
    vec2 p = v_position;
    vec2 dp = 0.7 * (v_position - glowCenter) / float(SAMPLES);
    float glow = 0.0;
    float weight = 1.0;
    for (int i = 0; i < SAMPLES; i++)
    {
        float data = texture(channel0, vec2(0.5) + 0.5 * p).a;
        glow += data * weight * mix(0.8, 1.0, random(uv));
        weight *= 0.95;
        p -= dp * mix(0.9, 1.0, random(uv));
    }
    ...
    f_color.rgb += 0.08*vec3(1.0-0.3*uv.y, 0.0, 0.5+0.5*uv.x);
    f_color.a = 1.0;
}
);
