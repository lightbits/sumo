...
void tick(Input io, float t, float dt)
{
    ...
    for (u32 i = 0; i < sim_steps; i++)
    {
        ship.v = 1.0f;
        sim_time += timestep;
        // xd = -8000.0f + 16000.0f*io.mouse.pos.x / WINDOW_WIDTH;
        // yd = +8000.0f - 16000.0f*io.mouse.pos.y / WINDOW_HEIGHT;
    ...
    // vec2 p = vec2(-10.0f + 20.0f*io.mouse.pos.x/WINDOW_WIDTH,
    //               +10.0f - 20.0f*io.mouse.pos.y/WINDOW_HEIGHT);

    // lines_set_scale(1.0f / 10.0f);
    // lines_set_width(2.0f);
    // lines_set_color(vec4(1.0f, 0.5f, 0.1f, 1.0f));

    // vec2 pt = vec2(5.0f, 5.0f);
    // vec2 vt = normalize(vec2(1.0f, 1.0f))*3.0f;
    // vec2 n = normalize(vec2(1.0f, -1.0f));
    // vec2 pd = pt + n * 1.0f;
    ...
    // lines_draw_circle(pt, 0.2f);
    // lines_draw_line(pt, pt + vt);

    // lines_draw_circle(p, 0.2f);
    // lines_draw_line(p, p + vd);

    // lines_set_color(vec4(1.0f, 1.0f, 1.0f, 1.0f));
    // lines_draw_circle(pd, 0.1f);
    // lines_draw_circle(pd, D);
