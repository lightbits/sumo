// Random numbers between 0 and 64 (inclusive)
...
#define Seconds 1
#define Robot_Speed (330 * Millimeters / Seconds)
...
#define Top_Touch_Time (Reverse_Length / 4)
...
#define fsm_TransitionTo(state)            \
    {                                      \
    state##Start(event, internal, action); \
    return Robot_##state;                  \
    }

void ObstacleWaitStart(fsm_Event event, fsm_RobotInternalState *internal, fsm_Action *action)
{
    // printf("Transition to ObstacleWait\n");
    action->red_led = 1;
}

void ObstacleRunStart(fsm_Event event, fsm_RobotInternalState *internal, fsm_Action *action)
{
...
void ObstacleCollisionStart(fsm_Event event, fsm_RobotInternalState *internal, fsm_Action *action)
{
...
void TargetWaitStart(fsm_Event event, fsm_RobotInternalState *internal, fsm_Action *action)
{
    // printf("Transition to TargetWait\n");
    action->green_led = 1;
}

void TargetRunStart(fsm_Event event, fsm_RobotInternalState *internal, fsm_Action *action)
{
...
void TrajectoryNoiseStart(fsm_Event event, fsm_RobotInternalState *internal, fsm_Action *action)
{
    // printf("Transition to TrajectoryNoise\n");
    int offset = fsm_GetRandomInt() - Random_Max / 2;
    // printf("%d\n", offset);
...
void ReverseStart(fsm_Event event, fsm_RobotInternalState *internal, fsm_Action *action)
{
...
void TargetCollisionStart(fsm_Event event, fsm_RobotInternalState *internal, fsm_Action *action)
{
...
void TopTouchStart(fsm_Event event, fsm_RobotInternalState *internal, fsm_Action *action)
{
...
fsm_RobotState fsm_Update(fsm_RobotState state,
                          ...
                          fsm_Action *action)
{
    ...
    switch (state)
    {
        case Robot_Start:
        {
            ...
            {
                fsm_TransitionTo(TargetWait);
            }
            else
            {
                fsm_TransitionTo(ObstacleWait);
            }
        } break;
        ...
        case Robot_ObstacleRun:
        {
            ...
            else if (event.is_bumper)
            {
                action->left_wheel = 0.0f;
                action->right_wheel = 0.0f;
                fsm_TransitionTo(ObstacleCollision);
            }
        } break;
